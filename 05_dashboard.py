"""
Delivery Insights Dashboard - Streamlit App
...
def main():
    ...
    with col2:
        st.metric("⏱️ On-Time Rate", f"{kpis['on_time_rate']:.1f}%",
                  delta=f"Target: 95%")
    ...
    with tab1:
        ...
        with col2:
            ...
            fig = px.line(
                hourly_sat, x='hour', y='customer_satisfaction',
        ...
        fig = px.bar(
            daily_volume, x='date', y='deliveries',
    ...
    with tab2:
        ...
        **Legend:**
        - 🔵 Blue markers: Depots
        - 🟢 Green dots: Successful deliveries
        - 🔴 Red dots: Failed deliveries
        """)
    ...
    with tab3:
        ...
        st.dataframe(driver_perf.sort_values('Success Rate', ascending=False), 
        ...
        fig = make_subplots(rows=1, cols=3, 
                           subplot_titles=('Success Rate', 'Satisfaction', 'Avg Duration'))

        fig.add_trace(go.Bar(x=exp_perf['experience_level'], 
                            y=exp_perf['is_delivered'], name='Success %'), row=1, col=1)
        fig.add_trace(go.Bar(x=exp_perf['experience_level'], 
                            y=exp_perf['customer_satisfaction'], name='Satisfaction'), row=1, col=2)
        fig.add_trace(go.Bar(x=exp_perf['experience_level'], 
                            y=exp_perf['delivery_duration_min'], name='Duration'), row=1, col=3)

        fig.update_layout(height=400, showlegend=False, 
                         title_text="Performance by Experience Level")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        with col1:
            fig = px.bar(weather_impact, x='weather', y='is_delivered',
                        title="Success Rate by Weather", color='weather')
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            fig = px.bar(weather_impact, x='weather', y='delivery_duration_min',
                        title="Avg Duration by Weather", color='weather')
            st.plotly_chart(fig, use_container_width=True)

        traffic_impact = filtered.groupby('traffic_level').agg({
            'is_delivered': 'mean',
            'delivery_duration_min': 'mean'
        }).reset_index()
        traffic_impact['is_delivered'] = traffic_impact['is_delivered'] * 100

        fig = px.bar(traffic_impact, x='traffic_level', y=['is_delivered', 'delivery_duration_min'],
                    title="Impact of Traffic Level", barmode='group')
        st.plotly_chart(fig, use_container_width=True)
        ...
        try:
            ...
            **Model Performance:**
            ...
            with col1:
                ...
                experience = st.selectbox("Driver Experience", 
            ...
            st.warning("⚠️ Model not trained yet. Run eta_model.py first!")
