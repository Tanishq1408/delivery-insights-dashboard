...
class DeliveryDataGenerator:
    ...
    def generate_drivers(self, depots_df):
        ...
        for i in range(self.n_drivers):
            ...
            if experience == 'New':
                base_stops, accuracy = random.randint(25, 35), random.uniform(0.75, 0.85)
            elif experience == 'Intermediate':
                base_stops, accuracy = random.randint(35, 45), random.uniform(0.85, 0.90)
            elif experience == 'Experienced':
                base_stops, accuracy = random.randint(45, 55), random.uniform(0.90, 0.95)
            else:
                base_stops, accuracy = random.randint(55, 65), random.uniform(0.95, 0.98)
            drivers.append({
        ...
        for day in range(self.n_days):
            ...
            for _, depot in depots_df.iterrows():
                ...
                for _, driver in depot_drivers.iterrows():
                    n_deliveries = int(driver['base_stops_per_day'] * weekend_multiplier * peak_multiplier * random.uniform(0.8, 1.2))
                    for stop_num in range(n_deliveries):
                        ...
                        weather_delay = {'Sunny': 0, 'Cloudy': random.uniform(0, 5), 'Rainy': random.uniform(5, 15), 'Snowy': random.uniform(15, 30)}[weather]
                        traffic_delay = random.uniform(5, 20) if hour in [8, 9, 17, 18] else random.uniform(0, 10)
                        ...
                        success_prob = driver['delivery_accuracy'] - (0.05 if weather == 'Rainy' else 0) + (0.05 if is_weekend else 0)
